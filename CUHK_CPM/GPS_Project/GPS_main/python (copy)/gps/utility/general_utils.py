...
def save_hyperparams(filename, hyper):
...
def print_hyperparams(hyper, flag=0, l=0):
    for (key, value) in hyper.items():
        if isinstance(value, dict):
            print ' '*(flag*4+l), key, ': {'
            print_hyperparams(value, flag+1, l+len(key)+1)
            print ' '*(flag*4+l+len(key)+3), '}'
        else:
            print ' '*(flag*4+l), key, ':', value