...
def delete_target_from_npz(filename, target):
    """
    Delete a target from npz dictionary.
    Args:
    ...
    if os.path.exists(filename):
        with np.load(filename) as f:
            tmp = dict(f)
    for _k in [_s for _s in tmp.keys() if '_%s_' % target in _s]:
        print 'delete key = %s' % _k
        del tmp[_k]
