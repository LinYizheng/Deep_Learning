...
Action Panel                contains buttons for stop, reset, collect, fail
...
class GPSTrainingGUI(object):
    ...
    def __init__(self, hyperparams):
        ...
        self.request = None                 # Requests: stop, reset, collect, fail, None.
        self.err_msg = None
        self._colors = {
            'run': 'cyan',
            'wait': 'orange',
        ...
        # Actions.
        if not self._hyperparams.get('test_mode', False):
            actions_arr = [
                Action('stop',  'stop',  self.request_stop,  axis_pos=0),
                Action('reset', 'reset', self.request_reset, axis_pos=1),
                Action('collect', 'collect',    self.request_collect,    axis_pos=2),
            ...
            actions_arr = [
                ...
                Action('reset', 'reset', self.request_reset, axis_pos=1),
                Action('collect', 'collect', self.request_collect,    axis_pos=2),
                Action('test', 'test',   self.request_test, axis_pos=3),
            ]
        ...
        if config['image_on']:
            if not self._hyperparams.get('test_mode', False):
                ...
                self._test_traj_visualizer = Plotter3D(self._fig, self._gs_image_visualizer, num_plots=1)
        ...
        if self._hyperparams.get('test_mode', False):
            self._test_traj_visualizer.set_title(0, 'Testing trajectory')
    ...
    def request_collect(self, event=None):
        self.request_mode('collect')

    def request_fail(self, event=None):
        self.request_mode('fail')

    def request_test(self, event=None):
        self.request_mode('test')

    def request_mode(self, request):
        """
        Sets the request mode (stop, reset, collect, fail). The request is read by
    ...
    def update_test_traj(self, sample):
        # Plot testing trajectory
        self._test_traj_visualizer.clear(0)
        ee_pt = sample.get(END_EFFECTOR_POINTS)
        data = []
        for i in range(ee_pt.shape[1]/3):
            self._test_traj_visualizer.plot_3d_points(0, ee_pt[:, 3*i+0:3*i+3], color='blue', label='Policy Samples')
