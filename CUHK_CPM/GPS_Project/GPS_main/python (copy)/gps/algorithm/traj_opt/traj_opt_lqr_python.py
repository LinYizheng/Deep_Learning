...
class TrajOptLQRPython(TrajOpt):
    ...
    def update(self, m, algorithm):
        ...
        # eta is L-multiplier for prev & new lG pol
        ...
        # kl_step is epsilon
        # allowed difference between prev & new lG pol
        kl_step = algorithm.base_kl_step * step_mult
        print "KL Step:", kl_step
        ...
        for itr in range(DGD_MAX_ITER):
            # backward considers l(xt,ut) & BADMM with non-linear pol
            # get new lG pol(Kt, kt)
            traj_distr, new_eta = self.backward(prev_traj_distr, traj_info,
                                                prev_eta, algorithm, m)
            # forward: given dynamics and new lG pol get xt distribution
            new_mu, new_sigma = self.forward(traj_distr, traj_info)
            plt.plot(np.arange(T), new_mu[:, 0], 'b')
            ...
            if (abs(prev_eta - eta) < THRESHA and
                ...
                baseLogger.info("Iteration %i, KL: %f / %f converged (eta limit)",
                             itr, kl_div, kl_step * T)
                break
            ...
            if (itr > 2 and abs(kl_div - prev_kl_div) < THRESHB and
                    kl_div < kl_step*T):
                baseLogger.info("Iteration %i, KL: %f / %f converged (no change)",
                             itr, kl_div, kl_step * T)
                break

            prev_kl_div = kl_div
            baseLogger.info('Iteration %i, KL: %f / %f eta: %f -> %f',
        ...
        plt.plot(np.arange(T), new_mu[:, 0], 'r')
        # plt.show()
        return traj_distr, eta, new_mu
        ...
        while fail:
            ...
            for t in range(T - 1, -1, -1):
                ...
                try:
                    ...
                    baseLogger.info('LinAlgError: %s', e)
                    fail = True
                    break
            ...
            if fail:
                old_eta = eta
                eta = eta0 + del_
                baseLogger.info('Increasing eta: %f -> %f', old_eta, eta)
