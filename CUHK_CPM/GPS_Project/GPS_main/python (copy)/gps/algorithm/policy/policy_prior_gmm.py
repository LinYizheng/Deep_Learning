...
class PolicyPriorGMM(object):
    ...
    def update(self, samples, policy_opt, all_samples, retrain=True):
        ...
        X, obs, meta = samples.get_X(), samples.get_obs(), samples.get_meta()
        all_X, all_obs = all_samples.get_X(), all_samples.get_obs()
        U, all_meta = all_samples.get_U(), all_samples.get_meta()
        ...
        if self._hyperparams['keep_samples']:
            ...
            if self.meta is None:
                ...
                self.meta = np.concatenate([self.meta, meta], axis=0)
            # Remove excess samples from dataset.
            start = max(0, self.X.shape[0] - self._max_samples + 1)
            self.X = self.X[start:, :, :]
            self.obs = self.obs[start:, :, :]
            self.meta = self.meta[start:]
            # Evaluate policy at samples to get mean policy action.
            # samples are all samples, not only the current iteration
            print 'GMM update --> policy_opt.prob'
            Upol = policy_opt.prob(self.obs.copy(), self.meta[:])[0]
            # Create dataset.
            # N is all samples number for one condition
            N = self.X.shape[0]
            # dO = dX + dU
            ...
            print 'GMM update --> policy_opt.prob'
            all_U = policy_opt.prob(all_obs.copy(), all_meta[:])[0]
            N = all_X.shape[0]
