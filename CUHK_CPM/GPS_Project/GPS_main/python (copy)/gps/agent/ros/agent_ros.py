...
class AgentROS(Agent):
    ...
    def reset_arm(self, arm, mode, data):
        ...
        #TODO: Maybe verify that you reset to the cor.rect position.
    ...
    def sample(self, policy, condition, ee_pts=None, verbose=True, save=True):
        """
        ...
        trial_command.ee_points = ee_points.reshape(ee_points.size).tolist() if not ee_pts else ee_pts[0]
        trial_command.ee_points_tgt = \
                self._hyperparams['ee_points_tgt'][condition].tolist() if not ee_pts else ee_pts[1]
