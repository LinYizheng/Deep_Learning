...
JOINTS_MAX = 6
JOINTS = 6  # CHANGE THE NUMBER OF JOINTS HERE!
...
# global lock for enquiry operation, in case of two enqs arrive together, and mess up each other's parameter zone
ENQ_SERVICING = 213
...
get_code = {"GO": GO, "TGO": TGO, "MOVE": MOVE, "ARC": ARC, "JUMP": JUMP, "STOP": STOP, "GO_DEG": GO_DEG, \
            ...
            "TARGET_OK": TARGET_OK, "EXEC_TIME": EXEC_TIME, "QUIT": QUIT, "JUMP3": JUMP3,
            "JUMP_WITH_HIGH_SPEED": JUMP_WITH_HIGH_SPEED, \
            "CONV_SPEED": CONV_SPEED, "CONV_ON": CONV_ON, "CONV_CLEAR": CONV_CLEAR, "CONV_POS": CONV_POS, "HAND": HAND,
            "MOVE_OK": MOVE_OK}
...
get_type = {"GO": "float", "TGO": "float", "MOVE": "float", "ARC": "float", "JUMP": "float", "STOP": "none", \
            "SPEED": "int", "ACCEL": "int", "POWER": "int", "MOTOR": "int", "INPOS": "none", "GO_DEG": "float",
            "ELBOW": "int", \
            ...
            "CONV_SPEED": "int", "CONV_ON": "int", "CONV_CLEAR": "int",
            "HAND": "int"}  # Jump3 not listed, 2 different types
    ...
    while not count_i[0] == count_re[0]:
        ...
        if (kcnt % 100000 == 0):
            ...
            with open("./VS_log.txt", 'a+') as f:
                f.write((str(datetime.now()) + "Stuck 1: count_i = " + str(count_i[0]) + "count_re = " + str(
                    count_re[0]) + "\n"))
...
def get_status(shm_obj, enq_str, param_list=None):  # For normal enquiry other than "WHERE"
    # 20170118: wait if previous enq is not finished yet
    timer2 = 0
    while (shm_read(shm_obj, shm_size, ENQ_SERVICING, 1, 1)[0]):
        time.sleep(0.00005)  # 50us
        timer2 += 1
        if timer2 % 10000 == 0:
            print "Stuck of [", enq_str, "] Waiting for Last Enq to finish!..."
    shm_write(shm_obj, 1, shm_size, ENQ_SERVICING, 1, 1)
    ...
    #     enq_result = uchar2ulong(uc_result, 8) # form to an ulong
        ...
        if shm_read(shm_obj, shm_size, ENQ_RESPOND_FLAG, 1, 1)[0] == 1:
            ...
            with open("./VS_log.txt", 'a+') as f:
                f.write(
                    str(datetime.now()) + " [WARNING:] " + enq_str + " Respond flag ready before waiting for it!!! \n")
        ...
        kcnt = 0
        while shm_read(shm_obj, shm_size, ENQ_RESPOND_FLAG, 1, 1)[
            0] == 0:  # add[0], 20160705, wait for result to be updated by the other side
            time.sleep(0.00005)  # 50us
            kcnt += 1
            if (kcnt % 100000 == 0):
        ...
        # Jason add 20161014, finish
    shm_write(shm_obj, 0, shm_size, ENQ_SERVICING, 1, 1)
    return enq_result
