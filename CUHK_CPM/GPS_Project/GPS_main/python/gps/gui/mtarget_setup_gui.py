...
Action Panel                contains buttons for:
                            - add/delete target numbers,
                            - switching pr2 and gps controllers
...
from gps.gui.util import load_pose_from_npz, load_data_from_npz, \
        save_pose_to_npz, save_data_to_npz, delete_target_from_npz
...
from pr2_controller_manager_interface import *
...
#           Action('nat', 'next_actuator_type', self.next_actuator_type, axis_pos=3),
            ...
            Action('atn', 'switch_plugins',    self.switch_plugins,    axis_pos=2),
            Action('dtn', 'delete_target_number', self.delete_target_number, axis_pos=3),

            Action('sip', 'set_initial_position', self.set_initial_position, axis_pos=4),
            Action('sii', 'set_initial_image',    self.set_initial_image,    axis_pos=5),
            Action('stp', 'set_target_position',  self.set_target_position,  axis_pos=6),
...
#           Action('mm',  'mannequin_mode',   self.mannequin_mode,   axis_pos=11),
        ...
        if config['image_on']:
            ...
            self._image_visualizer = ImageVisualizer(self._fig,
                    ...
                    rostopic=config['image_topic'], show_overlay_buttons=False)
    ...
    def prev_target_number(self, event=None):
        assert stop_controllers(self.pr2_plugins) == True, 'fail to stop pr2 plugins'
        assert start_controller('GPSPR2Plugin') == True, 'fail to start gps plugin'
        self.plugin_mode = 'gps'
    ...
    def switch_plugins(self, event=None):
        self.set_action_status_message('switch_plugins', 'requested')
        if self.plugin_mode == 'gps':
            # disable GPS plugin and disable PR2 plugins
            assert stop_controller('GPSPR2Plugin') == True, 'fail to stop gps plugin'
            assert start_controllers(self.pr2_plugins) == True, 'fail to start pr2 plugins'
            self.plugin_mode = 'pr2'
            #assert start_controllers('head_traj_controller') == True, 'fail to start head_traj_controller'
        else:
            # enable GPS plugin and disable PR2 plugins
            assert stop_controllers(self.pr2_plugins) == True, 'fail to stop pr2 plugins'
            assert start_controller('GPSPR2Plugin') == True, 'fail to start gps plugin'
            self.plugin_mode = 'gps'
        self.set_action_status_message('switch_plugins', 'completed', message='plugin switched to %s' % self.plugin_mode)

    def delete_target_number(self, event=None):
        assert stop_controllers(self.pr2_plugins) == True, 'fail to stop pr2 plugins'
        assert start_controller('GPSPR2Plugin') == True, 'fail to start gps plugin'
        self.plugin_mode = 'gps'
        self.set_action_status_message('delete_target_number', 'requested')
        delete_target_from_npz(self._target_filename, self._target_number)
        self.reload_positions()
        self.update_target_text()
        self.set_action_status_message('delete_target_number', 'completed',
                message='target number = %d deleted' % self._target_number)
    ...
    def set_initial_position(self, event=None):
        ...
        try:
            for _ in range(5):
                _ = self._agent.get_data(arm=self._actuator_type)
            #XXX workaround: get it twice to avoid stale data
        ...
        # XXX unmark below to reset arm after setting initial pos
        self.reload_positions()
        self.update_target_text()
    ...
    def set_target_position(self, event=None):
        ...
        #XXX unmark below to reset arm after set target
        self.reload_positions()
        self.update_target_text()
