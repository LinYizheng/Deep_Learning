...
from gps.algorithm.dynamics.dynamics_utils import guess_dynamics, guess_dynamics_rr
...
def init_lqr(hyperparams):
    ...
    # only one iteration
    # initial traj is all zeros
    # Ltt = np.diag(np.hstack((config['stiffness'] * np.ones(dX), np.ones(dU))))
    # Ltt = Ltt / config['init_var']  # Cost function - quadratic term.
    # lt = -Ltt.dot(np.r_[x0, np.zeros(dU)])  # Cost function - linear term.

    # u = x
    # factor = 0.9
    # Ltt = np.diag(np.hstack(((1-factor)*np.ones(dX), factor*np.ones(dU))))
    # lt = -Ltt.dot(np.r_[x0, x0.copy()])

    # u = delta x
    factor = 1
    Ltt = np.diag(np.hstack((np.ones(dX), factor * np.ones(dU))))
    # Ltt = np.diag(np.hstack((np.ones(dU), np.zeros(dX - dU), factor * np.ones(dU))))
    ...
    for t in range(T - 1, -1, -1):
        ...
        # LU decomposition, default return U
        # U.T*U = L*L.T = A = LU
...
def init_lqr_rr(hyperparams):
    """
    Return initial gains for a time-varying linear Gaussian controller
    that tries to hold the initial position.

    Return initial gains for a time-varying linear Gaussian controller
    that tries to hold the initial position using 1-iteration DDP while nominal traj is all zeros
    """
    print 'zx, init_lqr_rr'
    config = copy.deepcopy(INIT_LG_LQR)
    config.update(hyperparams)

    x0, dX, dU = config['x0'], config['dX'], config['dU']
    tgt = config['tgt']
    print 'DDP trys to hold:', x0
    ...
    for t in range(T - 1, -1, -1):
        temp = x0+(tgt-x0)/float(T-1)*float(t)
        lt = -Ltt.dot(np.r_[temp, np.zeros(dU)])
        # Compute Q function at this step.
        if t == (T - 1):
            Ltt_t = config['final_weight'] * Ltt
            lt_t = config['final_weight'] * lt
