...
class AlgorithmBADMM(Algorithm):
    ...
    def iteration(self, sample_lists, data, pred_traj):
        """
        ...
        # Use iteration-based interpolation to set values of some schedule-based parameters.
        # set up ent_reg(PolOptCaffe) & lg_step(AlgorithmBADMM)
        ...
        self._update_step_size(data)  # KL Divergence step size.

        # Run inner loop to compute new policies.
        time_total = 0
        for inner_itr in range(self._hyperparams['inner_iterations']):
            print 'inner iteration', inner_itr
            #TODO: Could start from init controller.
            if self.iteration_count > 0 or inner_itr > 0:
                # Update the policy.
                # Compute the new CAFFE policy.
            ...
            if self.iteration_count > 0 or inner_itr > 0:
                ...
                for m in range(self.M):
                    print 'policy dual step'
                    self._policy_dual_step(m, step=step)

            # Compute new linear Gaussian controllers. considers all 4 items
            debugger.info("Inner loop iteration: %d", inner_itr)
            self._update_trajectories(pred_traj)

        print 'inner loop complete, advance variables'
    ...
    def _set_interp_values(self):
        ...
        # self._hyperparams['iterations'] is outer-loop iterations total number
        # e.g. iter = 10
        # t = 1/9, 2/9, ... , 8/9, 1, 1
        t = min((self.iteration_count + 1.0) /
                (self._hyperparams['iterations'] - 1), 1)
        # Perform iteration-based interpolation of entropy penalty.
        # ent_reg_schedule: [0.001  0.001  0.01   0.1]
        # print type(self._hyperparams['ent_reg_schedule'])
        if type(self._hyperparams['ent_reg_schedule']) in (int, float):
            ...
            # set hyperparams of PolicyOpt
            # ent_reg from 0.001 to 0.001 to 0.01 to 0.1
        ...
        if type(self._hyperparams['lg_step_schedule']) in (int, float):
            ...
            # lg_step_schedule : [ 0.0001  0.001   0.01    0.1   ]
    ...
    def _update_step_size(self, costData):
        """ Evaluate costs on samples, and adjust the step size. """
    ...
    def _update_policy(self, itr, inner_itr):
        ...
        meta_shape = list(self.cur[0].sample_list.get_meta().shape)
        meta_shape[0] = 0
        meta_data = np.zeros(tuple(meta_shape))
        for m in range(self.M):
            ...
            for t in range(T):
                ...
                # first ':' means all samples for concerning condition
            ...
            meta_data = np.concatenate((meta_data, samples.get_meta()))
        self.policy_opt.update(meta_data, obs_data, tgt_mu, tgt_prc, tgt_wt,
                               itr, inner_itr)
    ...
    def _update_policy_fit(self, m, init=False):
        ...
        # for one condition, current iteration
        samples = self.cur[m].sample_list
        N = len(samples)
        # pol_info is PolicyPriorGMM
        pol_info = self.cur[m].pol_info
        X = samples.get_X()
        # non-linear policy: pol_mu and pol_sig(cannot be calculated, set by default)
        print 'update policy fit --> policy_opt.prob'
        pol_mu, pol_sig = self.policy_opt.prob(samples.get_obs().copy(), samples.get_meta().copy())[:2]
        pol_info.pol_mu, pol_info.pol_sig = pol_mu, pol_sig
        # Update policy prior.
        # each condition has its own policy prior
        # considering not just current iteration
        if init:
    ...
    def _policy_dual_step(self, m, step=False):
        ...
        print 'policy dual step --> policy kl'
    ...
    def _stepadjust(self, m):
        ...
        baseLogger.info('Previous cost: Laplace: %f MC: %f',
                     np.sum(prev_laplace_obj), prev_mc_obj)
        baseLogger.info('Predicted new cost: Laplace: %f MC: %f',
                     np.sum(new_pred_laplace_obj), new_mc_obj)
        baseLogger.info('Actual new cost: Laplace: %f MC: %f',
                     np.sum(new_actual_laplace_obj), new_mc_obj)
        baseLogger.info('Previous KL: Laplace: %f MC: %f',
                     np.sum(prev_laplace_kl), np.sum(prev_mc_kl))
        baseLogger.info('Predicted new KL: Laplace: %f MC: %f',
                     np.sum(new_pred_laplace_kl), np.sum(new_mc_kl))
        baseLogger.info('Actual new KL: Laplace: %f MC: %f',
                     np.sum(new_actual_laplace_kl), np.sum(new_mc_kl))
        baseLogger.info('Previous w KL: Laplace: %f MC: %f',
                     prev_laplace_kl_sum, prev_mc_kl_sum)
        baseLogger.info('Predicted w new KL: Laplace: %f MC: %f',
                     new_pred_laplace_kl_sum, new_mc_kl_sum)
        baseLogger.info('Actual w new KL: Laplace %f MC: %f',
                     new_actual_laplace_kl_sum, new_mc_kl_sum)
        baseLogger.info('Predicted/actual improvement: %f / %f',
    ...
    def _policy_kl(self, m, prev=False):
        ...
        print 'policy kl --> policy_opt.prob'
        pol_mu, _, pol_prec, pol_det_sigma = self.policy_opt.prob(obs.copy(), samples.get_meta()[:])
        # Compute KL divergence.
        for t in range(T):
