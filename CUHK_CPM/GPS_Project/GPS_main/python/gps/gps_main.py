...
class GPSMain(object):
    ...
    def run(self, itr_load=None, testing=False):
        ...
        # zx, self._hyperparams[iterations] == 12
        for itr in range(itr_start, self._hyperparams['iterations']):
            time_total_start = time.time()

            sampleData = np.zeros((self._conditions, self._hyperparams['num_samples'], self._hyperparams['agent']['T'],
                                   self.agent.sensor_dims[JOINT_ANGLES] * 3))  # joint angles: 6, ee pose: 6, actions: 6
            pred_traj = np.zeros((self._conditions, self._hyperparams['agent']['T'],
                                  self.agent.sensor_dims[JOINT_ANGLES] * 2))  # joint angles: 6, actions: 6
            # zx, sampleData.shape == (40, 3, 100, 12), for 4 axis, joint angles: 4, ee pose: 4, actions: 4
            # zx, pred_traj.shape == (40, 100, 8), for 4 axis, joint angles: 4, actions: 4
            time_sample_start = time.time()
            # zx, self._train_idx == self._train_idx = [0, 1, 2, ..., 37, 38, 39]
            ...
            #print 'zx, time_sample[{}] = {}'.format(itr, time_sample[itr])
...
def main():
    ...
    # zx, args.targetsetup == False
    # zx, test_policy_N == None
    # zx, args.test == None
