...
SENSOR_DIMS = {
    RR_COORD6D: 4,  # 6
    JOINT_ANGLES: 4,  # 6
    JOINT_VELOCITIES: 4,  # 6
    ACTION: 4,  # 6
    XPLUS: 8,  # XPLUS = JOINT_ANGLES + target, six axis = 12
    RGB_IMAGE: 1
}
...
EXP_DIR = BASE_DIR + '/../experiments/ja_mode_02242017_170140.experiment/'
...
full_seq = [48, 15, 37, 16, 11, 46, 29, 21, 13, 39, 44, 42, 41,  4, 36, 47, 33,
       32, 17, 40,  5, 18, 14, 38, 10,  0,  2, 23, 28, 30, 31, 20,  3, 35,
        1, 43, 22, 25, 34, 19, 12, 26,  9,  8, 24,  6, 45,  7, 27]

test_seq = [8, 10, 12, 22, 24, 26, 36, 38, 40]
train_seq = filter(lambda x: x not in test_seq, full_seq)

# Set up each condition.
print common['target_filename']
for j in xrange(common['conditions']):
    i = train_seq[j]
