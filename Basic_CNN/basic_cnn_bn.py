...
'''
# print first element of the trainImg_array
firstTrainImg = trainImg_arr[0].img
firstTrainImg_label = trainImg_arr[0].label


#display image
print('Print first Train image')

showImg(firstTrainImg,firstTrainImg_label)


# print first element of the testImg_array
firstTestImg = testImg_arr[0].img
firstTestImg_label = testImg_arr[0].label


#display image
print('Print first Test image')

showImg(firstTestImg,firstTestImg_label)
'''
...
'''
So seems like Python bool can't be use in our batch_norm function. We need to create a placeholder for is_training
'''
...
# Define a batch normalization
def batch_norm(x, n_out, is_training):
    '''
    Argument:
    x: Tensor 4D input maps
    n_out: integer, depth of input maps
    phase_train: boolean tf.Variable, True for training phase
    scope: string, Variable scope

    Return:
    normed: batch_normalized maps
    '''

    #with tf.variables_scope('bn'):
...
def conv_net(x, weights, biases, dropout, is_training):
    ...
    # Apply Batch Norm during Training
    if is_training == True:
        conv1 = batch_norm(conv1,32, is_training)
    ...
    if is_training == True:
        conv2 = batch_norm(conv2,64, is_training)
...
pred_train = conv_net(x, weights, biases, keep_prob, is_training)

# Define loss and optimizer
cost_train = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred_train, labels=y))
optimizer_train = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost_train)

# Evaluate model
correct_pred_train = tf.equal(tf.argmax(pred_train, 1), tf.argmax(y, 1))
...
#This block of code may not be needed


pred_test = conv_net(x, weights, biases, keep_prob, False)

# Define loss and optimizer
cost_test = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred_test, labels=y))
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        sess.run(conv_net, feed_dict={x: batch_x, y:batch_y, keep_prob: dropout, is_training: True})
        sess.run(optimizer_train, feed_dict={x: batch_x, y: batch_y,
                                       keep_prob: dropout})
        if step % display_step == 0:
            # Calculate batch loss and accuracy
            loss, acc = sess.run([cost_train, accuracy_train], feed_dict={x: batch_x,
                                                              y: batch_y,
                                                              keep_prob: 1.})
    ...
    print("Testing Accuracy:", \
        sess.run(accuracy_test, feed_dict={x: test_img_matrix,
                                      y: test_label_matrix,
